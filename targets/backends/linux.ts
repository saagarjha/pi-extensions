...
import { searchScript, spawnSearch } from "../search.ts";
...
export class LinuxBackend implements TargetBackend {
	...
	private vmFromLabels(labels: Record<string, string> | null | undefined): Vm | undefined {
		try {
			if (labels?.[LABEL] !== "true" || labels[`${LABEL}.kind`] !== "vm") return undefined;
			const rawId = labels[`${LABEL}.id`];
			if (!rawId) return undefined;
			const id = safePart(rawId);
			const name = labels[`${LABEL}.name`];
			return { id, ...(name ? { name: safePart(name) } : {}), published: labels[`${LABEL}.published`] === "true" };
		} catch {
			return undefined;
		}
	}
	...
	private vmFromImage(image: string): Vm | undefined {
		try {
			return this.vmFromLabels(JSON.parse(docker(["image", "inspect", image, "--format", "{{json .Config.Labels}}"])) as Record<string, string> | null);
		} catch {
			return undefined;
		}
	}
	...
	listVms(): Vm[] {
		try {
			const imageIds = docker(["image", "ls", "--quiet", "--filter", `label=${LABEL}=true`, "--filter", `label=${LABEL}.kind=vm`])
				.split("\n")
				.filter(Boolean);
			if (imageIds.length === 0) return [];
			const labelSets = docker(["image", "inspect", ...imageIds, "--format", "{{json .Config.Labels}}"])
				.split("\n")
				.filter(Boolean);
			return labelSets.flatMap((raw) => {
				try {
					const vm = this.vmFromLabels(JSON.parse(raw) as Record<string, string> | null);
					return vm ? [vm] : [];
				} catch {
					return [];
				}
	...
	start(id0: string, opts: StartOptions): Promise<RunningTarget> {
		if (opts.sip) throw new Error("SIP state is only supported by macOS VMs");
		const id = safePart(id0);
		const image = vmId(id);
		if (!dockerOk(["image", "inspect", image])) throw new Error(`VM ${id} does not exist`);
		assertPiResource(image);
		...
		for (const m of opts.mounts) {
			args.push("--mount", `type=bind,src=${m.hostPath},dst=${guestPath(m.logicalHostPath ?? m.hostPath)}${m.mode === "ro" ? ",readonly" : ""}`);
		}
		...
		const target: RunningTarget = {
			...
			kind: "linux",
			vm: this.getVm(id) ?? { id },
			mounts: opts.mounts.map((m) => ({ ...m, guestPath: guestPath(m.logicalHostPath ?? m.hostPath) })),
	...
	fileOperations(targetId: string): FileOperations {
		const backend = this;
		return {
			searchPlan: path => ({ roots: [{ path, exclude: [] }], spawn: (tool, commands, signal) => spawnSearch("docker", [
				"exec", "--workdir", "/", requireContainer(targetId), "/usr/bin/env", "-i",
				"PATH=/usr/local/bin:/usr/bin:/bin", "HOME=/var/empty", "LANG=C.UTF-8", "/bin/sh", "-c", searchScript(tool, commands, tool),
			], { cwd: "/" }, signal) }),
			exists: async (path, signal) => (await backend.vmExecBytes(targetId, `if test -e ${shQuote(path)} || test -L ${shQuote(path)}; then printf y; else printf n; fi`, { signal })).toString("ascii") === "y",
			size: async (path, signal) => {
				const out = await backend.vmExecBytes(targetId, `wc -c < ${shQuote(path)}`, { signal });
				const size = Number(out.toString("ascii").trim());
				if (!Number.isSafeInteger(size) || size < 0) throw new Error(`Container returned an invalid file size for ${path}`);
				return size;
			},
			// Keep Linux's existing container semantics: symlinks are not directories
			// for metadata. Use the async container transport so cancellation reaches
			// the docker process; parsing is the small local adapter needed by tools.
			stat: async (path, signal) => {
				const out = await backend.vmExecBytes(targetId, `if test -L ${shQuote(path)}; then printf l; elif test -d ${shQuote(path)}; then printf d; elif test -e ${shQuote(path)}; then printf f; else exit 1; fi`, { signal });
				const marker = out.toString("ascii");
				const type = marker === "d" ? "directory" : marker === "l" ? "symlink" : "file";
				return { type, isDirectory: () => type === "directory" };
			},
			lstat: async (path, signal) => {
				const q = shQuote(path);
				const out = await backend.vmExecBytes(targetId, `if test -L ${q}; then printf l; elif test -d ${q}; then printf d; elif test -f ${q}; then printf f; elif test -e ${q}; then printf o; else exit 1; fi`, { signal });
				const marker = out.toString("ascii");
				const type = marker === "l" ? "symlink" : marker === "d" ? "directory" : marker === "f" ? "file" : "other";
				return { type, isDirectory: () => type === "directory" };
			},
			mkdir: async (path, signal) => { await backend.vmExecBytes(targetId, `mkdir -- ${shQuote(path)}`, { signal }); },
			readdir: async (path, signal) => splitNul(await backend.vmExecBytes(targetId, `find ${shQuote(path)} -mindepth 1 -maxdepth 1 -printf '%f\\0'`, { signal })).sort(),
			async *openRead(path, signal) { yield await backend.vmExecBytes(targetId, `cat -- ${shQuote(path)}`, { signal }); },
			async write(path, chunks, signal, onProgress) {
				const content: Buffer[] = [];
				let transferred = 0;
				for await (const chunk of chunks) { content.push(chunk); transferred += chunk.length; onProgress?.(transferred); }
				const slash = path.lastIndexOf("/");
				const parent = slash < 0 ? "." : slash === 0 ? "/" : path.slice(0, slash);
				await backend.vmExecBytes(targetId, `mkdir -p -- ${shQuote(parent)} && cat > ${shQuote(path)}`, { input: Buffer.concat(content), signal });
			},
		};
	}
