export type TargetKind = "local" | "linux" | "macos" | "remote";
export type Mount = { hostPath: string; guestPath: string; mode: "ro" | "rw"; /** Logical host namespace behind a session-owned policy proxy. */ logicalHostPath?: string; permissionSession?: string };
export type SipState = "enabled" | "disabled";
export type Vm = { id: string; name?: string; kind?: "linux" | "macos"; sip?: SipState; published?: boolean };
...
export type MacOSVmOptions = {
	/** Fixed RAM allocation for a macOS VM. macOS guests do not balloon memory. */
	ramMiB?: number;
};

export type CreateVmOptions = {
	/** VM operating system. Linux is intentionally the inexpensive default. */
	os?: "linux" | "macos";
	...
	base?: string;
	network?: boolean;
	/** Backend-specific creation options. */
	options?: MacOSVmOptions;
...
export type StartOptions = {
	...
	mounts: Mount[];
	network?: boolean;
	/** Desired SIP state for a macOS VM. Omit to retain its persisted state. */
	sip?: SipState;
	onOutput?: (output: string) => void;
};
