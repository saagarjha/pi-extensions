...
function hasActiveAsyncWork(): boolean {
	...
	return (bridge.backgroundActiveCount?.() ?? 0) > 0 || (bridge.subagentsActiveCount?.() ?? 0) > 0 || (bridge.goalActiveCount?.() ?? 0) > 0;
}
...
function collectUsage(ctx: ExtensionContext) {
	...
	const childTotals = idleStatusBridge().subagentsUsage?.();
	if (childTotals) {
	...
	ctx.ui.setFooter((_tui, theme, footerData) => ({
		...
		render(width: number): string[] {
			try {
				...
				const statusOrder = ["subagents", "background", "goal"];
				const extensionStatuses = Array.from(footerData.getExtensionStatuses().entries())
					.filter(([key]) => key !== "auth-account")
					.sort(([a], [b]) => {
						const aOrder = statusOrder.indexOf(a);
						const bOrder = statusOrder.indexOf(b);
						return (aOrder < 0 ? statusOrder.length : aOrder) - (bOrder < 0 ? statusOrder.length : bOrder) || a.localeCompare(b);
					})
					.map(([, text]) => text)
					.filter(Boolean);
				return extensionStatuses.length
					? [statsLine, truncatePlain(extensionStatuses.join("  "), width)]
					: [statsLine];
			} catch {
				const model = ctx.model;
...
export default function extension(pi: ExtensionAPI) {
	pi.registerMessageRenderer("model.empty-output", (_message, _options, theme: any) => ({
		invalidate() {},
		render: (width: number) => [truncatePlain(theme.fg("warning", "∅  Model returned no output"), width)],
	}));
	const setActivity = (event: any, ctx: ExtensionContext) => {
		const label = activityLabel(event);
		if (label !== undefined || event.type === "agent_end" || event.type === "agent_settled") ctx.ui.setWorkingMessage(label);
	};
	...
	pi.on("before_provider_request", (event: any, ctx: ExtensionContext) => setActivity({ ...event, type: "before_provider_request" }, ctx));
	pi.on("after_provider_response", (event: any, ctx: ExtensionContext) => setActivity({ ...event, type: "after_provider_response" }, ctx));
	...
	pi.on("message_end", (event: any, ctx: ExtensionContext) => {
		...
		const visible = (message.content ?? []).some((part: any) =>
			(part?.type === "text" && String(part.text ?? "").trim()) || part?.type === "toolCall",
		);
		if (!visible) {
			void pi.sendMessage({ customType: "model.empty-output", display: true, content: "Model returned no output" }, { triggerTurn: false });
		}
	});
