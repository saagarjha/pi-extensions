...
import { CombinedAutocompleteProvider, Container, Editor, Loader, Spacer, matchesKey, truncateToWidth, visibleWidth, wrapTextWithAnsi, type Component, type Focusable, type TUI } from "@earendil-works/pi-tui";
...
import { permissionsBridge, type PermissionSubset } from "../permissions/src/bridge.ts";
import { CHILD_SYSTEM_PROMPT, PARENT_PROVENANCE_GUIDELINE } from "./protocol.ts";
import { installSubagentMessaging, messageProvenance, userMessageDisplayText, type SubagentMessaging } from "./messaging.ts";
...
class SubagentManager {
	...
	autocompleteCommands() {
		// Child sessions load the same project prompts and most sibling extensions.
		// Keep the parent-only subagent monitor out of their editor suggestions.
	...
	/**
	 * Child sessions run without their own TUI, but extension commands still
	 * need a visible destination for notify(). Route every such notice into the
	 * selected child's chat/status stream rather than special-casing commands.
	 */
	...
	private notifyParent(agent: Subagent, message: string) {
		...
		try {
			void Promise.resolve(this.api.sendMessage({
	...
	private unloadRuntime(agent: Subagent, note?: string) {
		// Preserve the final child usage after disposing its runtime.
		updateUsage(agent);
		agent.unsubscribe?.();
		agent.unsubscribe = undefined;
		if (agent.session) {
			childBridge().instances.delete(agent.session.sessionId);
			// SDK dispose need not emit extension session_shutdown. Release the
			// permission subscription explicitly instead of retaining a dead child.
			permissionsBridge().instances.get(agent.session.sessionId)?.dispose?.();
		}
		agent.messaging?.dispose();
	...
	private refreshUi() {
		...
		const parts = [
			ctx.ui.theme.fg("accent", `[${agents.length} subagents`),
			counts.running ? ctx.ui.theme.fg("accent", `${counts.running} running`) : undefined,
			counts.idle ? ctx.ui.theme.fg("dim", `${counts.idle} idle`) : undefined,
			counts.failed ? ctx.ui.theme.fg("error", `${counts.failed} failed`) : undefined,
		].filter((part): part is string => part !== undefined);
		if (parts.length > 0) parts[parts.length - 1] += "]";
		ctx.ui.setStatus("subagents", agents.length === 0 ? undefined : parts.join(" • "));
		ctx.ui.setTitle("pi");
	...
	private async run(agent: Subagent, ctx: ExtensionContext) {
		...
		await loader.reload();
		// Use Pi's native append-only session format for child transcripts. These
		// archives intentionally are not restored as live agents after restart.
		...
		const { session } = await createAgentSession({
			...
			sessionManager: SessionManager.create(ctx.cwd, childSessionDir),
		});
		...
		await session.bindExtensions({ mode: "print", uiContext: this.childUiContext(agent, ctx) });
		...
		if (!parentPermissions.reduceSnapshot || !childPermissions.inheritFrom) {
			session.dispose();
			throw new Error("Permission inheritance failed closed: permissions bridge does not support live delegation. Reload the extensions.");
		}
		const delegatedPermissions = parentPermissions.reduceSnapshot(agent.permissions);
		childPermissions.inheritFrom(ctx.sessionManager.getSessionId(), delegatedPermissions);
		...
		try {
			await agent.messaging.sendParent(agent.instructions);
			if (agent.interruptRequested) {
				agent.interruptRequested = false;
				transitionTaskStatus(agent, "idle");
	...
	update(idOrName: string, params: { permissions?: PermissionSubset }, ctx: ExtensionContext) {
		...
		if (!parentPermissions.reduceSnapshot || !childPermissions.inheritFrom) throw new Error("Permission update failed: permissions bridge does not support live delegation. Reload the extensions.");
	...
	submitHumanInput(idOrName: string, text: string, delivery: Delivery = "auto") {
		return this.deliver(idOrName, text, delivery, "user");
	}

	private async deliver(idOrName: string, text: string, delivery: Delivery, origin: "parent" | "user") {
		const agent = this.getOpen(idOrName);
		if (!agent) throw new Error(`Unknown or closed subagent: ${idOrName}`);
		if (!agent.session || !agent.messaging) throw new Error(`Subagent runtime is not loaded: ${idOrName}`);
		const submit = origin === "user" ? agent.messaging.sendUser : agent.messaging.sendParent;
		...
		if (!active && this.isTerminal(agent)) throw new Error(`Subagent ${agent.name} is ${agent.status}; spawn a new subagent if needed.`);
		agent.dormant = false;
		this.changed();
		if (active) {
			const streamingBehavior = deliveryUsed === "followUp" ? "followUp" : "steer";
			await submit(text, streamingBehavior);
		} else {
			transitionTaskStatus(agent, "running");
			this.changed();
			// Parent-to-child delivery is mailbox delivery: do not keep the parent
			// tool call open for the child's entire turn.
			void submit(text).then(
				() => {
					if (agent.interruptRequested) {
						agent.interruptRequested = false;
						transitionTaskStatus(agent, "idle");
...
function activeSubagentWorkLabel(agent: Subagent): string | undefined {
	return agent.status === "starting" || agent.status === "running" ? agent.activity ?? "Working…" : undefined;
}
...
function pendingQueueLines(agent: Subagent, width: number, theme: ExtensionContext["ui"]["theme"]): string[] {
	const lines: string[] = [];
	for (const text of agent.queuedSteering) lines.push(...renderPlainBlock(`Steering: ${text}`, width, (line) => theme.fg("dim", line)));
	for (const text of agent.queuedFollowUp) lines.push(...renderPlainBlock(`Follow-up: ${text}`, width, (line) => theme.fg("dim", line)));
	return lines;
}
...
function subagentUserComponent(message: AgentSession["messages"][number], theme: ExtensionContext["ui"]["theme"]): Component {
	const container = new Container();
	const origin = messageProvenance(message)?.origin;
	if (origin) container.addChild(new LinesComponent([theme.fg("dim", origin === "user" ? "User" : origin === "parent" ? "Parent" : "Extension")]));
	container.addChild(new UserMessageComponent(userMessageDisplayText(message)));
	return container;
}
...
function renderTranscriptFallback(agent: Subagent, width: number, theme: ExtensionContext["ui"]["theme"], toolsExpanded: boolean, toolComponent: ToolComponentProvider): string[] {
	...
	for (const entry of transcript.slice(-100)) {
		...
		if (entry.kind === "user") {
			lines.push(...subagentUserComponent(entry.userMessage ?? { role: "user", content: text, timestamp: entry.time }, theme).render(width).map(stripTranscriptControlCodes));
	...
	if (messages.length === 0) {
		...
		for (const message of messages) {
			...
			if (message.role === "assistant") {
				...
				appendSeparated(lines, subagentUserComponent(message, theme).render(width).map(stripTranscriptControlCodes));
			} else {
		...
		for (const entry of agent.transcript.slice(-100)) {
			...
			if (entry.kind === "user") {
				...
				appendSeparated(lines, subagentUserComponent(entry.userMessage ?? { role: "user", content: text, timestamp: entry.time }, theme).render(width).map(stripTranscriptControlCodes));
...
function installTools(pi: ExtensionAPI) {
	...
	pi.registerTool(defineTool({
		...
		async execute(_id, params) {
			...
			return toolResult(lines.join("\n"), { agents: agents.map((agent) => ({ id: agent.id, name: agent.name, status: agent.status, dormant: Boolean(agent.dormant), model: modelLabel(agent.model), thinkingLevel: agent.thinkingLevel, instructions: agent.instructions, queuedSteering: agent.queuedSteering, queuedFollowUp: agent.queuedFollowUp })), display: lines.join("\n") });
		},
		renderResult: renderDisplayResult,
	}) satisfies ToolDefinition);
	...
	pi.registerTool(defineTool({
		...
		renderCall(params: any, theme: any) {
			return toolCallLine(["update_subagent", params?.id], theme);
		},
...
/** Keep Pi's colored top editor rule, but omit its bottom frame rule. */
class TopChromeEditor extends Editor {
...
class SubagentsPanel implements Component, Focusable {
	...
	constructor(
		...
		this.input = new TopChromeEditor(this.tui, {
			borderColor: (text) => this.ctx.ui.theme.fg("border", text),
			selectList: {
				selectedPrefix: (text) => this.ctx.ui.theme.fg("accent", text),
				selectedText: (text) => this.ctx.ui.theme.fg("accent", text),
				description: (text) => this.ctx.ui.theme.fg("muted", text),
				scrollInfo: (text) => this.ctx.ui.theme.fg("dim", text),
				noMatch: (text) => this.ctx.ui.theme.fg("warning", text),
			},
		});
		this.input.setAutocompleteProvider(new CombinedAutocompleteProvider(manager.autocompleteCommands(), this.ctx.cwd));
		this.input.onSubmit = (text) => { void this.sendSelected("auto", text); };
	...
	handleInput(data: string) {
		...
		if (data === "\x04" && !this.input.getText()) { this.dismissSelected(); this.tui.requestRender(); return; }
		// Ctrl-C should interrupt the selected subagent. When autocomplete is open,
		// let the editor consume its navigation/cancel keys before interpreting
		// them as panel controls.
		...
		if (
			...
			!this.input.getText() &&
			this.selectedHasQueuedMessages()
		) {
			this.dequeueSelected();
			this.tui.requestRender();
			return;
		}
		if (this.keybindings.matches(data, "tui.select.up") && !this.input.getText()) { this.selected = Math.max(0, this.selected - 1); this.scrollOffset = 0; }
		else if (this.keybindings.matches(data, "tui.select.down") && !this.input.getText()) { this.selected = Math.min(Math.max(0, agents.length - 1), this.selected + 1); this.scrollOffset = 0; }
		...
		else if (matchesKey(data, "shift+tab") && !this.input.getText()) { this.selected = agents.length === 0 ? 0 : (this.selected + agents.length - 1) % agents.length; this.scrollOffset = 0; }
		else if (this.keybindings.matches(data, "tui.input.tab") && !this.input.getText()) { this.selected = agents.length === 0 ? 0 : (this.selected + 1) % agents.length; this.scrollOffset = 0; }
		else if (this.keybindings.matches(data, "app.message.dequeue") && !this.input.getText()) this.dequeueSelected();
	...
	private selectedHasQueuedMessages() {
		const agent = this.selectedAgent();
		return Boolean(agent && (agent.queuedSteering.length || agent.queuedFollowUp.length));
	}
	...
	private async interruptSelected() {
		...
		if (agent.status !== "starting" && agent.status !== "running") {
			manager.note(agent.id, `${agent.name} is not running`);
			return;
		}
		try {
			const { steering, followUp } = await manager.stop(agent.id);
			const text = [...steering, ...followUp].join("\n\n");
			if (text) this.input.setText([text, this.input.getText()].filter(Boolean).join("\n\n"));
			manager.note(agent.id, `Interrupted${text ? "; restored queued messages to the editor" : ""}`);
		}
		catch (error) { manager.note(agent.id, error instanceof Error ? error.message : String(error), "error"); }
		this.tui.requestRender();
	}

	private async sendSelected(delivery: Delivery, value = this.input.getText()) {
		const agent = this.selectedAgent();
		if (!agent || !value.trim()) return;
		const text = value.trim();
		this.input.setText("");
		this.scrollOffset = 0;
		// The delivered message is rendered in the child transcript, just like a
		// normal chat user message; don't duplicate a send acknowledgement below
		// the editor.
		try { await manager.submitHumanInput(agent.id, text, delivery); }
		catch (error) { manager.note(agent.id, error instanceof Error ? error.message : String(error), "error"); }
		this.tui.requestRender();
	}

	private dequeueSelected() {
		const agent = this.selectedAgent();
		if (!agent) return;
		try {
			const { steering, followUp } = manager.dequeue(agent.id);
			const text = [...steering, ...followUp].join("\n\n");
			if (!text) {
				manager.note(agent.id, `${agent.name} has no queued messages`);
				return;
			}
			this.input.setText([text, this.input.getText()].filter(Boolean).join("\n\n"));
			manager.note(agent.id, `Restored ${steering.length + followUp.length} queued message${steering.length + followUp.length === 1 ? "" : "s"} to the editor`);
		} catch (error) {
			manager.note(agent.id, error instanceof Error ? error.message : String(error), "error");
		}
	}
	...
	private dismissSelected() {
		...
		try {
			...
			this.scrollOffset = 0;
			manager.note(agent.id, "Dismissed");
		} catch (error) {
			manager.note(agent.id, error instanceof Error ? error.message : String(error), "error");
		}
	}
	...
	private handleAgentEvent(agent: Subagent, event: AgentSessionEvent): void {
		...
		if (ev.type === "message_end" && ev.message?.role === "user") {
			// Origin labels are added at message_end, after Pi's queue bookkeeping.
			this.chatViews.delete(agent.id);
			this.tui.requestRender();
			return;
		}
	...
	private appendChatChild(container: Container, child: Component, needsSeparator: { value: boolean }, separate = false): void {
		if (separate && needsSeparator.value) container.addChild(new Spacer(1));
		...
		if (messages.length === 0) {
			...
			for (const message of messages) {
				...
				if (message.role === "assistant") {
					...
					this.appendChatChild(container, subagentUserComponent(message, this.ctx.ui.theme), needsSeparator, true);
				} else {
			...
			for (const entry of agent.transcript.slice(-100)) {
				...
				if (entry.kind === "user") {
					if (renderedUserTexts.has(text)) continue;
					this.appendChatChild(container, subagentUserComponent(entry.userMessage ?? { role: "user", content: text, timestamp: entry.time }, this.ctx.ui.theme), needsSeparator, true);
					renderedUserTexts.add(text);
				} else if (entry.kind === "system" || entry.kind === "error") {
					const color = entry.kind === "error" ? "error" : "dim";
	...
	private renderCachedChatLog(agent: Subagent, width: number, toolsExpanded: boolean): string[] {
		...
		if (!view || view.key !== key) {
			...
			view = {
				...
				needsSeparator: container.children.length > 0,
				key,
			};
	...
	private syncInputBorder(agent: Subagent | undefined): void {
		const level = agent?.thinkingLevel ?? "off";
		const colors: Record<ThinkingLevel, string> = {
		...
		this.input.borderColor = (text) => this.ctx.ui.theme.fg(colors[level] as any, text);
	}

	render(width: number): string[] {
		const agents = manager.list();
		this.pruneToolComponents(agents);
		this.syncInputBorder(agents[this.selected]);
		const w = width;
		...
		if (agents.length === 0) {
			...
			if (agent) {
				const model = `${modelLabel(agent.model)}${agent.thinkingLevel && agent.thinkingLevel !== "off" ? ` (${agent.thinkingLevel})` : ""}`;
				lines.push(
					this.ctx.ui.theme.fg("dim", `${agent.id} · `)
					+ this.ctx.ui.theme.fg("accent", model),
				);
				...
				if (agent.uiStatus) {
					logLines.push("", ...renderPlainBlock(
						agent.uiStatus.text,
						innerWidth,
						(line) => this.ctx.ui.theme.fg(agent.uiStatus?.kind === "error" ? "error" : "dim", line),
					));
				}
				...
				const pendingLines = pendingQueueLines(agent, innerWidth, this.ctx.ui.theme);
				const pendingHeight = pendingLines.length ? pendingLines.length + 1 : 0;
				const maxLogLines = Math.max(4, bodyHeight - lines.length - reservedForInput - pendingHeight);
				...
				if (pendingLines.length) {
					...
					const dequeueHint = this.keyHint("app.message.dequeue", "to edit queued messages");
					if (dequeueHint) lines.push(this.ctx.ui.theme.fg("dim", `↳ ${dequeueHint}`));
				}
			}
		}

		const renderedInput = this.input.render(innerWidth);
		const inputHeight = renderedInput.length + 1;
