...
import { Container, Input, Loader, Spacer, matchesKey, truncateToWidth, visibleWidth, wrapTextWithAnsi, type Component, type Focusable, type TUI } from "@earendil-works/pi-tui";
...
class SubagentManager {
	...
	private async run(agent: Subagent, ctx: ExtensionContext) {
		...
		const loader = new DefaultResourceLoader({
			...
			systemPromptOverride: () => `You are a focused Pi subagent.\n\nRules:\n- Follow the delegated instructions.\n- Be concise and evidence-driven.\n- Prefer file paths, line references, commands run, and concrete findings.\n- Do not ask the user questions unless explicitly messaged by the user.\n\nCritical parent-communication protocol:\n- The parent cannot see your ordinary assistant messages.\n- To communicate with the parent, you must call the notify_parent tool.\n- Treat the delegated task's completion criteria as requiring a notify_parent report.\n- When you have the requested result, call notify_parent with the result in its message argument.\n- If you are blocked, cannot continue autonomously, or need parent attention, call notify_parent with a concise explanation.\n- Do not put final results in ordinary assistant text.\n- Do not use notify_parent for routine progress; keep ordinary progress in your own transcript.`,
		});
		await loader.reload();
	...
	async send(idOrName: string, text: string, delivery: Delivery = "auto") {
		...
		agent.dormant = false;
		if (!active) appendTranscript(agent, "user", text);
		this.changed();
		if (deliveryUsed === "steer") await agent.session.steer(text);
		else if (deliveryUsed === "followUp" && active) await agent.session.followUp(text);
		else {
			transitionTaskStatus(agent, "running");
			this.changed();
			try {
...
function installTools(pi: ExtensionAPI) {
	pi.registerTool(defineTool({
		...
		promptGuidelines: [
			...
			"Subagents are actors, not promises. They notify the parent when they need attention or have a result; dismiss them when you are satisfied. Do not poll them with inspect_subagent.",
		],
	...
	pi.registerTool(defineTool({
		...
		description: "Debug snapshot of a subagent's current transcript/state. Use only for debugging or when the user explicitly asks; subagents notify the parent when attention is needed. Never use this as a progress check.",
		promptSnippet: "inspect_subagent: debug/audit snapshot of a subagent transcript/state",
		promptGuidelines: [
			"Never poll inspect_subagent. Use it only for debugging or when the user explicitly asks to inspect a subagent.",
...
class SubagentsPanel implements Component, Focusable {
	...
	handleInput(data: string) {
		...
		else if (this.keybindings.matches(data, "app.message.dequeue") && !this.input.getValue()) this.dequeueSelected();
	...
	private async interruptSelected() {
		...
		try {
			...
			if (text) this.input.setValue([text, this.input.getValue()].filter(Boolean).join("\n\n"));
			this.status = `interrupted ${agent.name}${text ? "; restored queued messages to the editor" : ""}`;
		}
	...
	private dequeueSelected() {
		...
		try {
			...
			const text = [...steering, ...followUp].join("\n\n");
			if (!text) {
				this.status = `${agent.name} has no queued messages`;
				return;
			}
			this.input.setValue([text, this.input.getValue()].filter(Boolean).join("\n\n"));
			this.status = `restored ${steering.length + followUp.length} queued message${steering.length + followUp.length === 1 ? "" : "s"} to the editor`;
		} catch (error) {
	...
	private handleAgentEvent(agent: Subagent, event: AgentSessionEvent): void {
		...
		if (ev.type === "message_start") {
			if (ev.message?.role === "assistant") {
				...
				// session.messages already contains this delivered queue item. Rebuild
				// from that canonical list rather than appending an optimistic copy.
		...
		if (messages.length === 0) {
			...
			for (const message of messages) {
				...
				if (message.role === "assistant") {
					...
					this.appendChatChild(container, new UserMessageComponent(contentText(message.content)), needsSeparator, true);
				} else {
			...
			for (const entry of agent.transcript.slice(-100)) {
				...
				if (entry.kind === "user") {
					if (renderedUserTexts.has(text)) continue;
					this.appendChatChild(container, new UserMessageComponent(text), needsSeparator, true);
					renderedUserTexts.add(text);
				} else if (entry.kind === "system" || entry.kind === "error") {
					const color = entry.kind === "error" ? "error" : "dim";
	...
	render(width: number): string[] {
		...
		if (agents.length === 0) {
			...
			if (agent) {
				...
				lines.push(this.ctx.ui.theme.fg("accent", `${agent.name}`) + this.ctx.ui.theme.fg("dim", `  ${agent.status}${pendingQueueLabel(agent)} · ${visibility} · ${agent.id}`));
