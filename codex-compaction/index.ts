...
function latestCodexDetailsBySummary(ctx: any): Map<string, any> {
	...
	for (const entry of entries) {
		if (entry?.type === "custom" && entry.customType === EXTENSION_ID && typeof entry.data?.compactionEntryId === "string") {
			checkpoints.set(entry.data.compactionEntryId, entry.data.codex);
		}
	}
	const map = new Map<string, any>();
	for (const entry of entries) {
		if (entry?.type !== "compaction") continue;
		// Keep checkpoints from sessions written by the previous implementation readable.
		const codex = checkpoints.get(entry.id) ?? entry.details?.[EXTENSION_ID]?.codex;
...
export default function extension(pi: ExtensionAPI) {
	...
	let pending: {
		...
		codex: { output: AnyRecord[]; model: { provider: string; id: string } };
	} | undefined;
	...
	pi.on("session_before_compact", async (event, ctx) => {
		...
		if (!codexModel || !isCodexModel(codexModel)) return;
		...
		ctx.ui.notify("Generating Codex remote checkpoint; Pi will generate the normal summary", "info");
		try {
			const payload = await captureCodexPayload(ctx, codexModel, messages, ctx.getSystemPrompt(), event.signal);
			if (event.preparation.previousSummary) {
				payload.input.unshift({ role: "user", content: [{ type: "input_text", text: event.preparation.previousSummary }] });
			}
			const output = await fetchCodexRemoteCompaction(ctx, codexModel, payload, event.signal);
			if (output?.length && !event.signal.aborted && attempt === currentAttempt) {
				pending = {
					sessionId,
					firstKeptEntryId: event.preparation.firstKeptEntryId,
					codex: { output, model: { provider: codexModel.provider, id: codexModel.id } },
				};
			}
		} catch (error) {
			if (!event.signal.aborted) ctx.ui.notify(`Codex checkpoint failed; continuing with Pi compaction: ${error instanceof Error ? error.message : String(error)}`, "warning");
		}
		// No custom compaction result: Pi owns summary generation, instructions,
		// split turns, file tracking, usage, and persistence.
	});

	pi.on("session_compact", (event, ctx) => {
		const checkpoint = pending;
		clearPending();
		if (!checkpoint || event.fromExtension || checkpoint.sessionId !== ctx.sessionManager.getSessionId()) return;
		const entry = ctx.sessionManager.getBranch().findLast((entry) => entry.type === "compaction");
		if (!entry || entry.firstKeptEntryId !== checkpoint.firstKeptEntryId) return;
		pi.appendEntry(EXTENSION_ID, { version: 4, compactionEntryId: entry.id, codex: checkpoint.codex });
	});

	pi.on("session_compact_failed", clearPending);
