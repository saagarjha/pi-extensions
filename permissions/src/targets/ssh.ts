...
export type RemoteSshStatus = {
	status: "starting" | "running" | "done" | "failed" | "killed" | "unknown" | "unreachable";
	exitCode?: number;
	output: string;
	error?: string;
};

export class SshJobContinuesError extends Error {
	constructor(message: string, readonly remote: RemoteSshJob) {
		super(message);
	}
}
...
function runSsh(target: SshTargetConfig, remoteCommand: string, opts: { stdin?: string; signal?: AbortSignal; onData?: (chunk: Buffer) => void; timeoutMs?: number } = {}): Promise<{ stdout: string; stderr: string; exitCode: number | null }> {
