...
import { DockerBackend } from "./targets/docker.ts";
import { followSshJob, launchSshJob, probeSshTarget, runSshBytes, shellQuote, SshJobContinuesError, statusSshJob, stopSshJob, type RemoteSshJob, type SshTargetConfig } from "./targets/ssh.ts";
...
function denialText(d: Denial): string {
	...
	lines.push("", "To allow this, add the needed permission with /permissions.");
	return lines.join("\n");
}
...
export default function extension(pi: ExtensionAPI) {
	...
	const state: State = { scopes: [], vms: [], execGrants: [], sshTargets: [], network: "deny", targets: [LOCAL] };
	...
	function sshRunningTarget(ssh: SshTarget): RunningTarget {
		return { id: ssh.id, kind: "ssh", vm: null, mounts: [], network: false, exec: true };
	}

	function sshConfig(id: string): SshTarget | undefined {
		return state.sshTargets.find((s) => s.id === id);
	}
	...
	function loadPermissionsState(ctx: ExtensionContext): void {
		...
		try {
			...
			if (Array.isArray(saved.sshTargets)) {
				...
				for (const ssh of state.sshTargets) state.targets.push(sshRunningTarget(ssh));
			}
	...
	function activeToolNames(verbs = availableVerbs(state).filter((v) => IMPLEMENTED.includes(v))) {
		const systemReadable = systemScopes.some((s) => canReadMode(s.mode));
		const systemWritable = systemScopes.some((s) => canWriteMode(s.mode));
		const effectiveVerbs = [...new Set([...verbs, ...(systemReadable ? ["read", "ls", "find", "grep"] as Verb[] : []), ...(systemWritable ? ["write", "edit"] as Verb[] : [])])];
		const hasReadable = effectiveVerbs.some((v) => v === "read" || v === "ls" || v === "find" || v === "grep");
		const hasWritable = effectiveVerbs.some((v) => v === "write" || v === "edit");
		const sshCopyTargets = state.targets.filter((t) => t.kind === "ssh" && state.execGrants.some((g) => g.target === t.id && g.command === "*"));
		const canCopy = (hasReadable && hasWritable) || sshCopyTargets.length > 0;
		const copy = canCopy ? ["copy"] : [];
		const background = effectiveVerbs.includes("bash") ? ["bg_start", "bg_list", "bg_status", "bg_stop"] : [];
		return [...ALWAYS_ON, ...effectiveVerbs, ...copy, ...background];
	}
	...
	function mergedActiveToolNames(verbs = availableVerbs(state).filter((v) => IMPLEMENTED.includes(v))) {
		const managed = managedPermissionToolNames();
		const preserved = pi.getActiveTools().filter((name) => !managed.has(name));
		return [...preserved, ...activeToolNames(verbs)];
	}
	...
	async function guardBash(ctx: ExtensionContext, params: { target: string; command: string }): Promise<GuardResult> {
		...
		if (t.kind === "ssh" && state.network === "ask") {
	...
	pi.registerTool({
		...
		label: "Capabilities",
		description:
			"Report exactly what this session may do right now: which directories were added and at what mode, which VMs are usable, which targets are running, and which tools are live. Use this when access is unclear, and after a permission denial.",
		promptSnippet: "Report what this session is currently permitted to do",
		promptGuidelines: [
			"Your current access is listed in the system prompt under 'Your access right now'. Treat it as accurate and act on it directly; do not claim you lack access that is listed there.",
			"Tool availability is dynamic. Tools appear when their prerequisites exist: file tools need readable/writable file or VM access; command tools need a running exec-capable target with an exec grant.",
			"If an expected tool is not listed, call capabilities to inspect the current targets, grants, live tools, and prerequisite state before concluding the task is impossible.",
			"Call capabilities when you want more detail than that summary, or after a denial.",
			"When a tool reports DENIED, do not retry variations — the refusal is structural. Use the denial/capabilities output to determine the missing target, path grant, mode, or user-provided access.",
		],
		...
		async execute(_id, _params, _signal, _onUpdate, ctx) {
			...
			const running = state.targets.map(
				(t) =>
					`  ${t.id}${t.exec ? " [exec-capable]" : ""}${t.kind === "docker" ? " [docker exec allowed]" : ""}${t.kind === "ssh" ? " [ssh]" : ""}${t.network ? " [network]" : ""}` +
			...
			const lines = [
				...
				`tools: ${mergedActiveToolNames(verbs).join(", ") || "none"}`,
			];
	...
	pi.registerTool({
		...
		async execute(_id, params, _signal, onUpdate, ctx) {
			...
			if (baseId && !vmScope(state, baseId) && !backend.isPublished(baseId)) {
				return bad(
			...
			try {
				...
				const vm = await backend.createVm({ name: createName, base: baseId, network: wantsNetwork, onOutput });
				state.vms.push({ vmId: vm.id, mode: "rw" });
				refreshSessionSystemPaths(ctx);
				const run = await backend.start(vm.id, {
	...
	pi.registerTool({
		...
		async execute(_id, params, _signal, _onUpdate, ctx) {
			...
			try {
				const run = await backend.start(vmId, {
					mounts: mountsForCurrentScopes(),
	...
	pi.registerTool({
		...
		async execute() {
			try {
				const vms = backend.listVms();
				return ok(vms.length ? vms.map((v) => {
					const run = state.targets.find((t) => t.vm?.id === v.id);
					return `${v.id}${v.name ? ` (${v.name})` : ""} ${run ? "running" : "stopped"}${backend.isPublished(v.id) ? " [published]" : ""}`;
				}).join("\n") : "(none)");
			} catch (err) {
	...
	pi.registerTool({
		...
		async execute(_id, params, _signal, _onUpdate, ctx) {
			...
			try {
				const published = backend.publish(t.id, publishId);
	...
	function checkLocated(loc: LocatedPath): void {
		...
		else if (!backend.vmExists(loc.targetId, loc.actual)) throw new Error(`No such file or directory: ${loc.actual}`);
	}

	function readLocated(loc: LocatedPath): Buffer {
		return loc.kind === "host" ? readFileSync(checkedPath(loc.actual, loc.allow)) : backend.vmReadFile(loc.targetId, loc.actual);
	}

	function writeLocated(loc: LocatedPath, content: string): void {
		if (loc.kind === "host") writeFileVerified(loc.actual, content, loc.allow);
		else backend.vmWriteFile(loc.targetId, loc.actual, content);
	}
	...
	registerBuiltInFsTool("ls", createLsToolDefinition, (t, ctx): LsOperations => ({
		...
		stat: (absolutePath) => {
			...
			return loc.kind === "host" ? statSync(checkedPath(loc.actual, loc.allow)) : backend.vmStat(loc.targetId, loc.actual);
		},
		readdir: (absolutePath) => {
			const loc = actualForBuiltIn(t, absolutePath, "ls", ctx.cwd);
			return loc.kind === "host" ? listDirVerified(loc.actual, loc.allow).entries : backend.vmReadDir(loc.targetId, loc.actual);
		},
	}));
	...
	registerBuiltInFsTool("find", createFindToolDefinition, (t, ctx): FindOperations => ({
		...
		glob: (pattern, searchPath, options) => {
			...
			if (loc.kind === "host") {
				...
				for (const entry of backend.vmWalk(loc.targetId, loc.actual, options.limit * 20)) visit(entry.path, entry.isDir);
			}
			return hits;
		},
	}));
	...
	function isDirLocated(loc: LocatedPath): boolean {
		return loc.kind === "host" ? statSync(checkedPath(loc.actual, loc.allow)).isDirectory() : backend.vmStat(loc.targetId, loc.actual).isDirectory();
	}

	function listLocated(loc: LocatedPath): string[] {
		return loc.kind === "host" ? readdirSync(checkedPath(loc.actual, loc.allow)).map(nfc).sort() : backend.vmReadDir(loc.targetId, loc.actual);
	}

	function mkdirLocated(loc: LocatedPath): void {
		if (loc.kind === "host") mkdirVerified(loc.actual, loc.allow);
		else backend.vmMkdir(loc.targetId, loc.actual);
	}
	...
	function writeBufferLocated(loc: LocatedPath, content: Buffer): void {
		...
		else backend.vmWriteFileBytes(loc.targetId, loc.actual, content);
	}
	...
	type CopyEndpoint = LocatedPath | { kind: "ssh"; cfg: SshTargetConfig; actual: string };
	...
	function locateCopyEndpoint(t: RunningTarget, path: string, verb: "read" | "write", cwd: string): CopyEndpoint {
		if (t.kind === "ssh") {
			...
			return { kind: "ssh", cfg, actual: nfc(isAbsolute(path) ? path : resolve("/", path)) };
		}
	...
	async function approveSshFilesystem(ctx: ExtensionContext, t: RunningTarget, op: "read" | "write", path: string): Promise<string | undefined> {
		if (t.kind !== "ssh") return undefined;
	...
	async function sshChecked(cfg: SshTargetConfig, command: string, stdin?: Buffer, signal?: AbortSignal): Promise<Buffer> {
		const res = await runSshBytes(cfg, command, { stdin, signal, timeoutMs: 300_000 });
	...
	async function vmChecked(targetId: string, command: string, stdin?: Buffer, signal?: AbortSignal): Promise<Buffer> {
		return backend.vmExecBytes(targetId, command, { input: stdin, signal, timeoutMs: 300_000 });
	}
	...
	async function existsCopyEndpoint(ep: CopyEndpoint, signal?: AbortSignal): Promise<boolean> {
		...
		if (ep.kind === "vm") {
			try {
				await vmChecked(ep.targetId, `if test -d ${shellQuote(ep.actual)}; then printf d; elif test -e ${shellQuote(ep.actual)}; then printf f; else exit 1; fi`, undefined, signal);
				return true;
			} catch { return false; }
		}
		const res = await runSshBytes(ep.cfg, `if test -d ${shellQuote(ep.actual)}; then printf d; elif test -e ${shellQuote(ep.actual)}; then printf f; else exit 1; fi`, { signal, timeoutMs: 300_000 });
	...
	async function isSymlinkCopyEndpoint(ep: CopyEndpoint, signal?: AbortSignal): Promise<boolean> {
		...
		if (ep.kind === "vm") {
			try {
				await vmChecked(ep.targetId, `test -L ${shellQuote(ep.actual)}`, undefined, signal);
				return true;
			} catch { return false; }
		}
		const res = await runSshBytes(ep.cfg, `test -L ${shellQuote(ep.actual)}`, { signal, timeoutMs: 300_000 });
		return res.exitCode === 0;
	}
	...
	async function isDirCopyEndpoint(ep: CopyEndpoint, signal?: AbortSignal): Promise<boolean> {
		...
		const command = `if test -L ${shellQuote(ep.actual)}; then exit 2; elif test -d ${shellQuote(ep.actual)}; then printf d; elif test -e ${shellQuote(ep.actual)}; then printf f; else exit 1; fi`;
	...
	async function isFileCopyEndpoint(ep: CopyEndpoint, signal?: AbortSignal): Promise<boolean> {
		...
		if (ep.kind === "vm") {
			try {
				await vmChecked(ep.targetId, `test -f ${shellQuote(ep.actual)}`, undefined, signal);
				return true;
			} catch { return false; }
		}
		const res = await runSshBytes(ep.cfg, `test -f ${shellQuote(ep.actual)}`, { signal, timeoutMs: 300_000 });
		return res.exitCode === 0;
	}

	async function listCopyEndpoint(ep: CopyEndpoint, signal?: AbortSignal): Promise<string[]> {
		if (ep.kind === "host") return listLocated(ep);
		const command = `find ${shellQuote(ep.actual)} -mindepth 1 -maxdepth 1 -printf '%f\\0' | sort -z`;
		const out = ep.kind === "vm" ? await vmChecked(ep.targetId, command, undefined, signal) : await sshChecked(ep.cfg, command, undefined, signal);
	...
	async function mkdirCopyEndpoint(ep: CopyEndpoint, signal?: AbortSignal): Promise<void> {
		if (ep.kind === "ssh") await sshChecked(ep.cfg, `mkdir -- ${shellQuote(ep.actual)}`, undefined, signal);
		else if (ep.kind === "vm") await vmChecked(ep.targetId, `mkdir -- ${shellQuote(ep.actual)}`, undefined, signal);
		else mkdirLocated(ep);
	}

	async function readCopyEndpoint(ep: CopyEndpoint, signal?: AbortSignal): Promise<Buffer> {
		if (ep.kind === "ssh") return sshChecked(ep.cfg, `cat -- ${shellQuote(ep.actual)}`, undefined, signal);
		if (ep.kind === "vm") return vmChecked(ep.targetId, `cat -- ${shellQuote(ep.actual)}`, undefined, signal);
	...
	async function writeCopyEndpoint(ep: CopyEndpoint, content: Buffer, signal?: AbortSignal): Promise<void> {
		if (ep.kind === "ssh") await sshChecked(ep.cfg, `test -d ${shellQuote(dirname(ep.actual))} && cat > ${shellQuote(ep.actual)}`, content, signal);
		else if (ep.kind === "vm") await vmChecked(ep.targetId, `test -d ${shellQuote(dirname(ep.actual))} && cat > ${shellQuote(ep.actual)}`, content, signal);
		else {
			if (signal?.aborted) throw new Error("aborted");
			let real: string;
			try {
		...
		async execute(_id, params, signal, _onUpdate, ctx) {
			...
			try {
				...
				return ok(`Copied ${files} file${files === 1 ? "" : "s"}${dirs ? ` and created ${dirs} director${dirs === 1 ? "y" : "ies"}` : ""}.`, {
					display: `${params.sourceTarget}:${params.sourcePath} → ${params.destTarget}:${params.destPath}\n${files} file${files === 1 ? "" : "s"}${dirs ? `, ${dirs} director${dirs === 1 ? "y" : "ies"}` : ""}`,
				});
			} catch (err) {
	...
	function ctxCwdFallback() { return process.cwd(); }
	...
	async function syncRemoteBackgroundJob(job: BackgroundJob, tailChars = 8000) {
		...
		if (!cfg) return;
		const remote = await statusSshJob(cfg, job.remote, tailChars);
	...
	async function stopBackgroundJob(job: BackgroundJob) {
		if (job.status === "running") {
			if (job.remote) {
				const cfg = sshConfig(job.target);
				if (cfg) {
					try { await stopSshJob(cfg, job.remote); }
	...
	pi.registerTool({
		...
		promptGuidelines: ["bash requires a running target with exec enabled. Use the targets listed by capabilities; if none has exec, create/start a VM or ask the user to provide an exec target.", "Foreground bash commands time out after at most 10 minutes. Requests for longer timeouts are rejected; use bg_start for commands that may run longer, then continue other work until its completion notification or inspect it with bg_status/bg_list."],
		parameters: Type.Object({
			target: targetParam,
			command: Type.String(),
		...
		async execute(id, params, signal, onUpdate, ctx) {
			...
			if (g.t.kind === "ssh") {
				...
				let output = "";
				try {
					const remote = await launchSshJob(cfg, jobId, params.command, cwd);
					const append = (chunk: Buffer) => {
						output = appendStreamingText(output, chunk.toString());
						onUpdate?.(ok(output) as any);
					};
					const result = await followSshJob(cfg, remote, { signal, timeoutMs, onData: append });
			...
			const operations: BashOperations = {
				exec: (command, cwd, options) => {
					...
					return g.t.kind === "local" ? localExecStream(command, opts) : backend.execStream(g.t.id, command, opts);
				},
			};
	...
	pi.registerTool({
		...
		promptGuidelines: ["Use bg_start for long-running commands such as dev servers, watchers, and test loops.", "After starting a background command, do meaningful non-overlapping work immediately instead of waiting by reflex.", "Do not poll bg_status. Use bg_list for mailbox/status inspection or /background for live output; completion notifications arrive asynchronously."],
		parameters: Type.Object({
			target: targetParam,
			command: Type.String(),
		...
		async execute(_id, params, _signal, _onUpdate, ctx) {
			const g = await guardBash(ctx, params);
			if (g.denial) return bad(denialText(g.denial));
			const sshCfg = g.t.kind === "ssh" ? sshConfig(g.t.id) : undefined;
			if (g.t.kind === "ssh" && !sshCfg) return bad(`ssh target not found: ${g.t.id}`);
			...
			if (g.t.kind === "ssh") {
				...
				const runBackground = g.t.kind === "local" ? localExecStream : backend.execStream.bind(backend, g.t.id);
	...
	pi.registerTool({
		...
		promptGuidelines: ["Use bg_list for occasional inspection only; completion notifications arrive asynchronously.", "Do not poll background status; continue useful work unless the user asks for inspection."],
		parameters: Type.Object({
	...
	pi.registerTool({
		...
		description: "Show status and buffered output for background commands. Use only for deliberate inspection of a specific job's output; do not poll it.",
		parameters: Type.Object({
	...
	async function offerRestartWithoutNetwork(ctx: ExtensionContext, vmIds?: Set<string>): Promise<string> {
		...
		for (const t of targets) {
			const vmId = t.vm?.id;
			if (!vmId) continue;
			await backend.stop(t.id);
			state.targets = state.targets.filter((x) => x.id !== t.id);
			refreshSessionSystemPaths(ctx);
			const run = await backend.start(vmId, { mounts: mountsForCurrentScopes(), network: false });
			state.targets.push(run);
	...
	function permissionCompletions(argumentPrefix: string) {
		...
		if (kind === "vm" && (tokens.length === 2 || tokens.length === 3)) {
			const vmIds = new Set([...state.vms.map((v) => v.vmId), ...backend.listVms().map((v) => v.id)]);
	...
	pi.on("before_agent_start", async (event, ctx) => {
		...
		const block = [
			...
			state.execGrants.length
				...
				: "\nExec grants: none.",
			`\nTargets running: ${state.targets.map((t) => `${t.id}${t.exec ? " [exec-capable]" : ""}${t.kind === "docker" ? " [docker exec allowed]" : ""}`).join(", ")}`,
			`Tools you can call right now: ${mergedActiveToolNames(verbs).join(", ") || "none — nothing has been added"}`,
			"",
			"Rules: tool availability is dynamic and follows current grants/targets; the user adds directories and VMs; you cannot grant yourself file/VM access the user has not provided.",
			"Network is separate permission. If you only need network temporarily, use it briefly, then stop/restart without network when done.",
			"Command execution requires a running exec-capable target plus an exec grant. Docker VM targets allow any command by design; non-VM targets require an exact command grant or *.",
			"File operations require a valid target plus a path covered by current readable/writable directory or VM access. If access is unclear, call capabilities or attempt the relevant file tool and follow its DENIED result.",
			"If a tool you expect is not listed, check capabilities: it may appear after the required path grant, VM grant, or exec target exists.",
		]
			.filter((l) => l !== "")
			.join("\n");
